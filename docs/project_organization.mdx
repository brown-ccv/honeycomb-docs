...
The `package.json` file contains metadata about your project and scripts to run tasks related to your task.

and `scripts` can be added as desired, but otherwise this file should not be edited manually. To remove or add a dependency use `npm install` or `npm uninstall` with the `-D` flag if installing a dev dependency.

### package-lock.json

The `package-lock.json` contains metadata about the package installation. It is updated when running `npm install` and should never be manually updated.
...
This folder contains different .env files with presets for common use cases. These files can be loaded with `dotenv`, which is explained in greater detail in the [Configuration](environment_variables) section.
