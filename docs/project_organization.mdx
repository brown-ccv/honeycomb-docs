...
The file `psiturk-it` inside `psiturkit/` is a bash script used to instal PsiTurk locally - see [PsiTurk](psiturk#build-instructions) for more information. `psiturkit/` should be left alone!
...
The `assets/` directory contains all of the images and videos in the application.
...
:::caution
You shouldn't have to touch any of the code in these files.
:::
...
- `cli.js` is the script used to download and delete data stored in Firestore.
