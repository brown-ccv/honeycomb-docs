...
Custom code used by the various deployments. This is where sending data to Firebase is handled
...
- `honeycombBlock.js` contains the timeline for the Honeycomb block. It uses the task settings from [config.json](config#configjson).
-
