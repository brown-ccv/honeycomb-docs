...
:::info
Firebase will update the files `firebase-hosting-pull-request.yml` and `firebase-hosting-merge.yml` inside the `.github/workflows/` directory.
:::

:::caution
Firebase has been known to not do this correctly, please ensure the "run" step is `npm ci && npm run build:firebase` in both files!
:::
