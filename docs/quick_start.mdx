...
The package manager installs Node Version Manager (nvm). NVM keeps track of multiple versions of NodeJS on your system at the same time. To install the version needed by Honeycomb:

1.  Copy the version number listed in `.nvmrc`. _The version may be different than the screenshot below._
...
3.  Use the needed version. This will automatically check the version listed in the `.nvmrc` file.

    ```shell
    nvm use
    ```

4.  Set the current version as your default
...
```shell title="Install the dotenv cli"
