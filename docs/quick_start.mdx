...
Chocolatey will install multiple software packages if given a configuration file. The honeycomb template repository includes one! See [Prerequisites](prerequisites#chocolatey) for more information about using chocolatey.
...
In addition to the packages installed by Chocolatey you need to complete your Python installation using `pyenv`. Please complete the steps in [Python on Windows](prerequisites#python-on-windows-pyenv) before continuing.
...
Open `package.json` and edit it to reflect your app:
...
3. `author` is your lab (or PIs) name, email, and webpage
...
Dependencies are packages that the project relies on for it to function properly.
...
HoneyComb tasks can be configured to run as a web app in Firebase, as desktop application via electron, to receive port signals from EEG, cameras, foot pedals and more. To learn more about how to configure your task for these different scenarios, see the [Configuration Section](environment_variables).

## 8. Check out the data

The data is saved throughout the task. When running as a Desktop App, data is saved to the user's app directory. When running as a Web App in Firebase, data is stored in the FireStore.


For a Desktop App, the location of the task is logged at the beginning of the task wherever you ran `npm run dev`. Data is also stored in a folder that is generated by the app, which should be found on the desktop.
