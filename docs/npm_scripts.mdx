...
Runs `npm start` and `npm run electron-dev` concurrently. This may not play nicely with Windows OS. If it doesn't, run `npm start` and `npm run electron-dev` from different terminal windows.
