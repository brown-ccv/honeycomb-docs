...
[git](https://git-scm.com) is an open-sourced version control system. It is used to track changes, revert mistakes, and enable peer code reviews!

[GitHub Desktop](https://desktop.github.com) is a GUI application used to interact with git and GitHub directly from your computer. It is not strictly needed but many folks find it easier to work with than using git directly from the command line.
...
[Java](https://www.oracle.com/java/) is another high-level programming languages that some tools  are written in (namely, the Firebase Emulators). We must install a JDK (Java Development Kit) for it to run.
