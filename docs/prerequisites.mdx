...
In addition, Honeycomb relies on Electron to package the cross-platform desktop applications. Some of electron's prerequisites are platform dependent - listed below are the prerequisites for each operating system along with resources for manually installing them.

## MacOS

### Brew

All of the required dependencies for Honeycomb on MacOS can be installed via the brewfile. See the [quick start](quick_start/#installing-prerequisites-with-homebrew-for-macos) for more information.
